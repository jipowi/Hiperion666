...
public class SoatBacking implements Serializable {
		...
		try {
			...
			if (!identificacion.equals("") && ValidatorCedula.getInstancia().validateCedula(identificacion)) {
				...
				if (cliente == null) {
					...
					ramoSoatBean.setNombreCliente(cliente.getNombrePersona() + " " + cliente.getApellidoPaterno() + " "
	...
	public void buscarContactoAseguradora() {
		...
		buscarContactoAseguradora(ramoSoatBean.getAseguradora());
	}
	...
	public void addAseguradora() {
		...
		try {
			Long idAseguradora = Long.parseLong(ramoSoatBean.getAseguradora());
			Aseguradora aseguradoraNew = aseguradoraService.consultarAseguradoraByCodigo(ramoSoatBean.getAseguradora());
			...
			AseguradoraDTO aseguradoraDTO = new AseguradoraDTO(detalleCatalogo.getDescDetCatalogo(), aseguradoraNew.getDirecion(),
					...
					ramoSoatBean.getContactoAseguradora());
	...
	public Poliza setearDatosPoliza() {
		...
		poliza.setRamo(RamoEnum.R21.getLabel());
	...
	public static void setAseguradorasDTO(List<AseguradoraDTO> aseguradorasDTO) {
		SoatBacking.aseguradorasDTO = aseguradorasDTO;
	}
